...
const initialKeyword = "Vertica Analytics";
const customerNames = [
  "KT",
  "LG Electronics",
...
async function expectStaticKeywordReady(page: Page) {
  ...
  await expect(keyword).toHaveText(initialKeyword);
...
test("desktop Hero carousel, pullback, handoff, and solution sequence", async ({ page }) => {
  ...
  await expect(page.locator(".keyword-text")).not.toHaveText(initialKeyword, { timeout: 5_000 });
