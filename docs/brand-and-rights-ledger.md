...
## 0. 2026-07-11 프로젝트 공개 노출 지시

사용자는 2026-07-11 이 Codex 작업에서 현재 자산의 **프로젝트 공개 사용·화면 노출을 허용한다**고 명시했다. 이 지시는 프로젝트 소유자의 제품 의사결정으로 기록하며, 구현과 로컬 RC 검증을 막던 내부 승인 gate는 통과한 것으로 본다.

이 기록은 다음을 의미하지 않는다.

- 고객 18개 또는 vendor 상표권자가 발급한 별도 허가서
- 계약상 허용 지역·기간·매체·철회 조건의 확인
- 현재 파일이 권리자가 제공한 official master라는 확인
- partnership, certification, tier 또는 endorsement claim의 승인
- GTG 공식 logo master, 공식 red, favicon, OG 또는 법인 표기의 확정

따라서 기존 ledger의 `public-use-approved` 정의는 유지한다. 각 자산의 관계 증거와 제3자 권리 상태를 승격하지 않고, 별도 `project-owner-approved` 사실만 추가한다. 고객 데이터도 `relationshipEvidenceLevel`, `projectOwnerDisplayApproval`, `thirdPartyRightsStatus`, `approvalReference`를 분리해 이 경계를 코드에 보존한다. `siteContent.isApproved`, `proofMode`, noindex, canonical과 release tag gate는 이번 지시만으로 변경하지 않는다.

## 1. 결론

- 현재 증거로 `public-use-approved`라고 판정할 수 있는 GTG, 고객, 제품 logo 파일은 **0개**다.
- 프로젝트 소유자의 현재 프로젝트 화면 노출 결정은 **승인**됐다. 이는 아래 자산별 외부 권리·official source 판정과 별개다.
...
- 2026-07-11 remediation에서 LoadRunner PNG의 화면 참조를 제거하고 text mark로 대체했다. 원본 파일은 삭제하지 않고 `docs/archive/2026-07/blocked-assets/pngaaa.com-5227511.png`로 이동해 public runtime 노출을 차단했다.
...
| `project-owner-approved` | 사용자가 현재 프로젝트의 화면 노출을 승인함. 제3자 권리자 허가나 official master를 뜻하지 않음 |
...
- 2026-07-10 pre-remediation `next start` probe에서는 `/hero/clients/01_kt.png`, `/hero/generated/customer-logos/01_kt.png`, `/hero/item-logo/pngaaa.com-5227511.png`, customer-card sample, item-logo SVG sample이 200이었다. 2026-07-11 remediation 뒤 LoadRunner 옛 public URL은 404이고 application request는 0건이다. 나머지 public asset의 HTTP 노출은 사용 허가 증거가 아니다.
...
현재 product mark는 각각 대응 Solution의 product reveal에 한 번만 노출된다. Vertica와 Confluent는 image mark, HashiCorp와 LoadRunner는 text mark다. Hero와 handoff에는 product mark가 없다.

| 사용 대상 | 저장소 경로 | 파일명 | SHA-256 | 원본 출처 | 공식 source 여부 | relationship 확인 | public-use approval | trademark/endorsement 위험 | 현재 화면 노출 | 상태 | 필요한 후속 조치 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| Vertica | `public/item-logo` | `3840px-Vertica_pos_blk_rgb.svg.webp` | `69bafc6bbd84945ef13c87d0f2618ca194939920f2f4f90b6628f767ab58d2f1` | filename/3840×2968 render가 [Wikimedia SVG](https://commons.wikimedia.org/wiki/File:Vertica_pos_blk_rgb.svg)와 일치하는 강한 2차-source 추정; repo provenance 없음. 추정 source는 CC BY-SA 4.0+attribution/share-alike 및 별도 trademark 검토가 필요하나 현재 이행 증빙 없음 | 아니오 | code가 [기술 blog](https://x2wizard.github.io/)를 링크할 뿐이며 현재 trademark owner, 계약, Rocket partner/reseller status의 증빙은 아님 | 프로젝트 노출 승인; 권리자 증빙 없음 | 매우 높음: 2026 인수 후 권리 주체 재확인 필요, logo 변형·별도 word overlay, affiliation 오해 | Solution 01 한 번 | `blocked` | `V-GATE`; Rocket을 포함한 현재 권리자, official master·permission·attribution 확인 |
| Confluent | `public/item-logo` | `3840px-Confluent,_Inc._logo.svg.webp` | `486df6047005b267df0c42920652be6e36a38ca6bf2476d9e868df110ff951ab` | filename/3840×743 render가 [Wikimedia SVG](https://commons.wikimedia.org/wiki/File:Confluent,_Inc._logo.svg) 계열과 일치하는 2차-source 추정; repo provenance 없음. 추정 source는 PD-textlogo로 설명하지만 별도 trademark 제한을 명시 | 아니오 | code가 [GTG Confluent URL](https://www.gtgsc.com/gtg/sub/confluent/platform.php)을 링크할 뿐이며 현재 계약/partner status 증빙은 없음; 2026 IBM 인수 후 실제 permission entity 확인 필요 | 프로젝트 노출 승인; 권리자 증빙 없음 | 매우 높음: 공식 지침은 express written permission·형식 유지·attribution 요구 | Solution 02 한 번 | `blocked` | `V-GATE`; Confluent/IBM 법무 기준 permission grantor와 official master 확인 |
| LoadRunner archived PNG | `docs/archive/2026-07/blocked-assets` | `pngaaa.com-5227511.png` | `a63ca4de6dfcfaf1bc338daa75f5e0266c5d70d76b537745813ed3048f19e8df` | [pngaaa detail 5227511](https://www.pngaaa.com/detail/5227511), 979×922, `Non-commercial Use` | 아니오 | code가 [GTG DevOps URL](https://www.gtgsc.com/gtg/sub/devops/load.php)을 링크할 뿐이며 OpenText 계약/partner status 증빙은 없음 | 현재 파일 사용 불가 | 매우 높음: 비공식 aggregator, stale product identity, affiliation 오해 | 이미지 미노출·request 0; Solution 04는 text mark | `blocked` / archived | archive 유지; official asset 승인 전 public 복귀 금지 |
| HashiCorp text mark | image asset 없음 | N/A | N/A | `solutions.ts` text와 CSS text mark | graphic official source 없음 | code가 [GTG HashiCorp URL](https://www.gtgsc.com/gtg/sub/hash/hash.php)을 링크할 뿐이며 계약/partner status evidence level은 `unknown`; 2025 IBM 인수 후 permission entity 확인 필요 | 프로젝트 노출 승인; 권리자 증빙 없음 | 중~높음: 반복·강조가 endorsement처럼 보일 수 있음 | Solution 03 한 번, 본문/CTA | `blocked` | 최소·정확한 nominative use와 attribution 검토; graphic logo 추가는 `V-GATE` 전 금지 |
...
- Vertica와 Confluent current image file은 권리 증거 확보 전 공개 release-ready로 승격하지 않는다. LoadRunner PNG는 public 밖 archive로 격리됐다.
...
- 현재 gate 결과: **BLOCKED**. 프로젝트 소유자 노출 승인은 완료됐지만 고객·vendor 권리자 증거는 미완료다. 2026-07-11 LoadRunner 옛 public URL은 404로 전환됐고 application request는 0건이다.
...
- Vertica와 Confluent image logo, archived LoadRunner PNG, 두 item-logo application sample.
