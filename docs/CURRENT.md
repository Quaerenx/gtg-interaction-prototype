...
- `prefers-reduced-motion`/mobile 판정 hook이 mount 후 갱신되므로, 첫 paint부터 동적 Canvas가 절대 생성되지 않는다는 엄격한 보장은 현재 테스트가 입증하지 않는다.
...
4. 승인된 `docs/DECISIONS.md`: 목표 제품 방향. 현재 문서는 아직 제안 상태다.
...
`docs/DECISIONS.md`의 B/C 대안, logo strap 제거, rail 변경은 이 기준선을 대체하려는 **새 제안**이다. 아직 구현되지 않았고 승인되지 않았으므로 과거 승인 이력을 자동으로 덮어쓰지 않는다.
