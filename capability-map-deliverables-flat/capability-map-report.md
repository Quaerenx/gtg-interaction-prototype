...
Desktop uses a wide landscape composition with a central spine, forked relationship paths, and five capability nodes visible at once.

Mobile uses a simplified vertical composition so the five capability names remain readable at 390px without horizontal overflow. The mobile SVG reduces dense labels and relies on the adjacent HTML list for the full semantic structure.
...
Labels are structural, such as `analytical layers`, `event handoff`, `provisioning mesh`, `validation gate`, and `diagnosis and action`.
